...
case class JokerHand(cards: String, bid: Long) {
  val cardsScore = List('A', 'K', 'Q', 'T', '9', '8', '7', '6', '5', '4', '3', '2', 'J').reverse.zipWithIndex.toMap.view.mapValues(_ + 1).toMap

  val groupedHand = {
    val charList = cards.toList

    val groupedHand = charList
      .groupBy((char: Char) => char)
      .values.toList
      .sortBy(groupList => {
        // Ensure sorted by largest group of cards first, then sort groups of same size by how high scoring the card is
    ...
    val groupedHandJokersRemoved = groupedHand.filterNot(_.head == 'J')

    val jokersIndex = groupedHand.indexWhere(_.head == 'J')
    val numJokers = if (jokersIndex == -1) 0 else groupedHand(jokersIndex).size
    if (groupedHandJokersRemoved.size == 0) {
      groupedHand
    } else {
      groupedHandJokersRemoved.head ++ Range(0, numJokers).map(_ => groupedHandJokersRemoved.head.head) :: groupedHandJokersRemoved.tail
    }
  }
...
def daySevenPartTwo(filename: String): Long = {
  val lines = Utils.fileLinesAs(filename, _.toList)
  val hands = lines.map(_.split(' ')).map(list => JokerHand(list(0), list(1).toLong))
  hands.sortBy(_.totalRank()).zipWithIndex.foldLeft(0L) { case (totalWinnings, (hand, rank)) =>
    totalWinnings + (hand.bid * (rank + 1))
  }
}
